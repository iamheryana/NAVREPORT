...
public class T30CostingD_HW3PS_AddWindow extends Window implements Serializable {
	private static final long serialVersionUID = 7826099855146654896L;
	
	// binding
	private T30CostingDHw3ps initValue;
	private T30CostingDHw3ps resultValue;
	
	private T29CostingH initT29CostingH;
	
	protected Bandbox cmbItemCategory;
	protected Listbox listItemCategory;
	...
	private Decimalbox decCogsSatuan;
	private Decimalbox decCogsTotal;
	
	private String modeTitle;
	
	private SelectQueryService selectQueryService;
	
	public static T30CostingDHw3ps show(Component parent, T30CostingDHw3ps t30CostingDHw3ps, T29CostingH t29CostingH, String modeTitle, SelectQueryService selectQueryService) {
		return new T30CostingD_HW3PS_AddWindow(parent, t30CostingDHw3ps, t29CostingH, modeTitle, selectQueryService).resultValue;
	}

	private T30CostingD_HW3PS_AddWindow(Component parent, T30CostingDHw3ps t30CostingDHw3ps, T29CostingH t29CostingH, String modeTitle, SelectQueryService selectQueryService) {
		super();
		this.initValue = t30CostingDHw3ps;
	...
	private void createBox() {
		...
		caption.setLabel("Hardware and 3rd Party Services - "+modeTitle);
		...
		Row row3 = new Row();
			new Label("Item Category").setParent(row3);
				
	
			cmbItemCategory = new Bandbox();
			cmbItemCategory.setMold("rounded");
			cmbItemCategory.setWidth("200px");
			cmbItemCategory.setAutodrop(true);
				Bandpopup popup1 = new Bandpopup();
					listItemCategory = new Listbox();
					//listItemCategory.setMold("paging");
					listItemCategory.setAutopaging(true);
					listItemCategory.setWidth("200px");
					listItemCategory.addEventListener(Events.ON_SELECT, selectItemCategory());
					listItemCategory.setParent(popup1);
				popup1.setParent(cmbItemCategory);
			
			if (modeTitle.equals("View")){
				this.cmbItemCategory.setDisabled(true);
			}
			cmbItemCategory.setParent(row3);
		...
		Row row4 = new Row();
			...
			if (modeTitle.equals("View")){
				this.cmbProductGroup.setDisabled(true);
			}
			...
			Groupbox groupboxDtl = new Groupbox();
				...
				Grid gridDtl = new Grid();
					...
					Rows rowsDtl = new Rows();
						...
						Row rowDtl4 = new Row();	
							new Label("COGS").setParent(rowDtl4);
							
							this.decCogsSatuan= new Decimalbox();
							this.decCogsSatuan.setFormat("#,##0");
							this.decCogsSatuan.setStyle("text-align:right;");	
							this.decCogsSatuan.addEventListener(Events.ON_CHANGE,
									changeNilaiListener());
							this.decCogsSatuan.setCols(20);
							this.decCogsSatuan.setMaxlength(20);
							if (modeTitle.equals("View")){
								this.decCogsSatuan.setReadonly(true);
							}
							this.decCogsSatuan.setParent(rowDtl4);	
							
							this.decCogsTotal= new Decimalbox();
							this.decCogsTotal.setFormat("#,##0");
							this.decCogsTotal.setStyle("text-align:right;");			
							this.decCogsTotal.setCols(20);
							this.decCogsTotal.setMaxlength(20);
							this.decCogsTotal.setReadonly(true);
							this.decCogsTotal.setParent(rowDtl4);								
						rowDtl4.setParent(rowsDtl);
	...
	private EventListener selectItemCategory() {
		return new EventListener() {

			@Override
			public void onEvent(Event event) throws Exception {
				
				cmbItemCategory.setValue(listItemCategory.getSelectedItem().getLabel());
				initValue.setItemCategory((String) listItemCategory.getSelectedItem().getValue());
				renderProductGroup();
				
				cmbItemCategory.close();
			}
		};
	}
	...
	public EventListener changeNilaiListener() {
		return new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				BigDecimal vCogsSatuan = new BigDecimal(0);
				if (CommonUtils.isNotEmpty(decCogsSatuan.getValue())) {
					vCogsSatuan = decCogsSatuan.getValue();
				}
				...
				BigDecimal vCogsTotal = new BigDecimal(0);
				vCogsTotal = vQty.multiply(vCogsSatuan);
				
				
				decSalesTotal.setValue(vSalesTotal);
				decCogsTotal.setValue(vCogsTotal);
	...
	private void init() {
		...
		ListBoxUtil.resetList(listItemCategory);
		Listitem vListItemCat = null;

		Listitem vListBlank =  listItemCategory.appendItem("<<<Please Select>>>", "");
		
		
		List<Object[]> vResult = selectQueryService.QueryItemCategoryCosting();
		if(CommonUtils.isNotEmpty(vResult)){
			for(Object[] aRslt : vResult){
				
				Listitem vList = listItemCategory.appendItem(aRslt[0].toString(),aRslt[1].toString());
				
				if (CommonUtils.isNotEmpty(initValue.getItemCategory())) {
					if (initValue.getItemCategory().equals(aRslt[1].toString())) {
						vListItemCat = vList;
					}  
				}
			}
		}
		if (vListItemCat == null){
			vListItemCat = vListBlank;
		}
		
		cmbItemCategory.setValue(vListItemCat.getLabel());
		listItemCategory.setSelectedItem(vListItemCat);
		...
		if (CommonUtils.isNotEmpty(initValue.getCogsSatuan())) {
			decCogsSatuan.setValue(initValue.getCogsSatuan());
		}
		
		if (CommonUtils.isNotEmpty(initValue.getCogsTotal())) {
			decCogsTotal.setValue(initValue.getCogsTotal());
		}
...
//		}
	...
	private void doSave() {
		...
		initValue.setCogsSatuan(decCogsSatuan.getValue());
		initValue.setCogsTotal(decCogsTotal.getValue());

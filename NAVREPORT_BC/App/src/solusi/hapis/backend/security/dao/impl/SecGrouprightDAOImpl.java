...
public class SecGrouprightDAOImpl extends BasisDAO<SecGroupright> implements SecGrouprightDAO {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<SecRight> getGrantedRight(Long id) {
		DetachedCriteria criteria = DetachedCriteria.forClass(SecRight.class);
		
		criteria.createCriteria("secGrouprights", "gr", CriteriaSpecification.INNER_JOIN);
		
		criteria.add(Restrictions.eq("gr.secGroup.id", id));

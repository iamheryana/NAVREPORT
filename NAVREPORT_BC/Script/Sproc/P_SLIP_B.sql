/****************************************
Name sprocs	: P_SLIP_B
Create by	: Hermansyah
Date		: 30-08-2002
Description	: Proses Untuk Report Slip FORM-B
Call From	: Main Proc
Sub sprocs	: -
Update by 	: Yudi's  -->  26/09/02
BY PEGGY 2008 11 26 : NAMBAHIN TGL PAYROLL DI REPORT 
*****************************************/
CREATE OR REPLACE FUNCTION P_SLIP_B  (	l_Periode DATE,
			    		l_UkerFr  VARCHAR(8),
			    		l_UkerTo  VARCHAR(8),
			    		l_FCab    VARCHAR(4),	
			    		l_TCab    VARCHAR(4),
...
DECLARE l_Para			VARCHAR(4000);		l_LineTXT		VARCHAR(4000);
	...
	l_M08SkCaba		VARCHAR(40);		l_M02Singkatan		VARCHAR(20);
	...
	l_S01RkBnk		VARCHAR(20);		l_M14Keterangan		VARCHAR(20);
	...
	l_TEMPSkCabang		VARCHAR(40);		l_TEMPUnitUsaha		VARCHAR(4);
	...
	l_TEMPNip		VARCHAR(10);       	l_TEMPFlagDppt		VARCHAR(1);
        l_TEMPNomFormat		VARCHAR(2);		l_TEMPNama		VARCHAR(25); 
	...
	l_TEMPKeterangan	VARCHAR(40);		l_TEMPNilai		DECIMAL(15,2);
	...
	l_KetTaxCal		VARCHAR(40);		l_NilTaxCal		DECIMAL(15,2);
	...
	l_FlgDpPt		VARCHAR(1);		l_S01Hari		DECIMAL(4,1);
	l_M10Harian		INT;            	l_S02KdDppt      	VARCHAR(4);
...
ON S02.TglPayr=S01.TglPayr AND
   S02.NIP    =S01.NIP
--
LEFT JOIN S0BLSTX S0B
ON S0B.TglPayr=S01.TglPayr AND
   S0B.NIP=S01.NIP
--
     ...
     --      
	  ...
	  FROM l_Temp 
  	  WHERE Tahun=EXTRACT(YEAR FROM l_S02TglPayr) AND 
	        ...
	        Nip  =l_S01NIP    AND  
	        FlagDppt=l_S02FlgDpPt AND  
	        NomFormat=l_M19NomFormat::varchar(2) ) > 0 THEN 
         --
            UPDATE l_Temp
...
$$ LANGUAGE plpgsql ;
/*
SELECT * FROM P_SLIP_B 
('2013-12-04', ' ', 'Z', ' ', 'Z', ' ', 'Z', 'U', 'U',

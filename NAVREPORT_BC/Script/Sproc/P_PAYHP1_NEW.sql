...
*****************************************/
--
DROP FUNCTION  P_PAYHP1(l_TglProses  DATE,
			...
			l_usr_id     INTEGER);
--
CREATE FUNCTION  P_PAYHP1(l_TglProses  DATE,
			l_LokasiFr   VARCHAR(4),
			l_LokasiTo   VARCHAR(4), 
			l_ukerFr     VARCHAR(8),
			l_ukerTo     VARCHAR(8), 
...
DECLARE l_S05FlagTHR 	VARCHAR(1);       	l_S05TglAkhir   DATE;
	...
	l_Curr_Income  	DECIMAL(15,2);  	l_Curr_kolom8    DECIMAL(15,2); 	
	l_JnsPeg       	VARCHAR(1);
...
SELECT JKK,      JHT,      JKM,      JPK,      NlTunjAnak,      Personalia,      FlgGGol,     TunjIst,       TunjAnk
INTO   l_FZ1JKK, l_FZ1JHT, l_FZ1JKM, l_FZ1JPK, l_FZ1NlTunjAnak, l_FZ1Personalia, l_FZ1FlgGGol, l_FZ1TunjIst, l_FZ1TunjAnk
FROM FZ1FLDA;
...
--BEGIN TRAN
----------- MULAI PROSES DATA -----------------
OPEN l_LOOP_M15 FOR SELECT M15.NIP,M15.Nama,M15.TglLahir,M15.PrdAwl,M15.TglKeluar,
...
--          WHERE NIP=l_M15NIP 
	  ...
	  WHERE NIP = l_M15NIP AND FLGDPPT = 'D' AND KDDPPT ='BSAL' AND 
	       TANGGAL BETWEEN COALESCE(l_S05TGLAKHIR,l_M15TglMasuk) + INTERVAL '1 day' AND l_TGLPROSES AND FLAG = 1 ;

	  SELECT COALESCE(SUM(JMLABSN),0) 
	  INTO   l_PlusHari 
	  FROM T02ABSN 
	  WHERE NIP = l_M15NIP AND FLGDPPT = 'D' AND KDDPPT ='BSAL' AND 
	       TANGGAL BETWEEN COALESCE(l_S05TGLAKHIR,l_M15TglMasuk) + INTERVAL '1 day' AND l_TGLPROSES AND FLAG = 0 ;
...
-- END BY PEGGY 2007 01 10 : PEGAWAI HARIAN DIPROSES HRD JUGA DONKZ 
                    ...
                    -- Gaji Pokok
                    SELECT * from fn_GajiHarian(l_TglProses,  
					l_S05TglAkhir,
					l_M15NIP, 
					l_M15PrdTetap,     
					l_M15GajiPerc,
...
--			  RETURNING S01_ID AS l_S01_ID ; 
			   ...
			   INTO    l_M03Singkatan, l_M03UsComp, l_M03Kolom, l_M03NoAcc, l_M03Status, 
				  ...
				  INTO l_TunjIstri;
				  SELECT * FROM fn_Vround(((l_FZ1TunjIst*l_GajiRp)/100)/fn_GetKurs(l_M03KdCurr,l_TglProses))
				  INTO l_TunjIstriVal; 
                          ELSE 
				  SELECt * FROM fn_Vround(l_FZ1NlTunjIst)
				  INTO l_TunjIstri;
				  SELECT * FROM fn_Vround(l_FZ1NlTunjIst/fn_GetKurs(l_M03KdCurr,l_TglProses))
				  INTO l_TunjIstriVal;                           
                          END IF; 
                          
                         -- Insert/Update Tunjangan Istri
                         IF  (SELECT COUNT(NIP) FROM S02DGAJ WHERE TglPayr=l_TglProses AND FlgDpPt='D' AND COALESCE(FlgAngs,' ')=' ' AND 
							    NIP=l_M15NIP AND KdDpPt='TK01') > 0 THEN 
                            BEGIN
                              UPDATE S02DGAJ
...
--                     END IF;                        
		    ...
		    END IF; 
                    ...
                    IF l_FZ1FlgPiut='Y' AND l_FlagKhusus='T' AND 
                     ...
                     END IF;    
                    -- Lembur
                    IF --l_FZ1FlgLemb='Y' AND 
			(SELECT COUNT(NIP) FROM T01LEMB WHERE NIP=l_M15NIP) > 0 THEN 
                       BEGIN
                     ...
                     END IF;
                    -- Jamsostek
                    IF l_AkhirBln=1 AND COALESCE(l_M15KPA,' ')<>' ' AND l_M15TglKPA<=l_TglProses AND l_FlagKhusus='T' THEN
                       BEGIN
                          -- Dapat JKK
			  ...
			  END IF; 
                    ...
                    -- Gross Current Income
                     ...
                     -- Variable
                     SELECT COALESCE(l_Curr_Income,0)+COALESCE(SUM(fn_KPusat(l_M15NIP,EncNilai,l_Mypass) ::DECIMAL(15,2)),0)  
                     INTO   l_Curr_Income
                     FROM S02DGAJ
...
-- BY PEGGY 2008 04 02 
			   ...
			   --
                           ...
                           SET EncTotInc     =fn_kcabang(l_M15NIP,l_EncTotInc ::VARCHAR(17),l_MyPass),
                               ...
                               FLAGH         =CASE WHEN l_AkhirBln=1 THEN ' ' ELSE '1' END 
                           WHERE  TglPayr=l_TglProses  AND  NIP=l_M15NIP; 
                           --
			   -- update summary pindah cabang hanya pada akhir bulan aja. 
			   IF l_AkhirBln = 1 THEN 
			   BEGIN 
			       l_Bulan := CASE  EXTRACT(MONTH FROM l_TglProses) --(CASE WHEN EXTRACT(MONTH FROMl_TglProses)-1=0 THEN 1 ELSE EXTRACT(MONTH FROMl_TglProses)-1 END) 
			        ...
			        l_Tahun := EXTRACT(YEAR FROM l_TglProses);
				     ...
				     END IF; 
                           ...
                           --*
			   END;  
			   END IF; 			   -- END IF l_AKHIRBLN = 1 
                       END; 
                    END IF; -- END IF S01 GAK ADA DETAIL                
                    --*             
                  -- End Of Filter
                  END ; 
               END IF;     

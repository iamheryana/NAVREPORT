/****************************************
Name sprocs	: P_SLIP_A
Create by	: Hermansyah
Date		: 19-03-2002
Description	: Proses Untuk Report Slip 
Call From	: Main Proc
Sub sprocs	: -
*****************************************/
CREATE OR REPLACE FUNCTION  P_SLIP_A  (	l_Periode DATE,
			    		l_UkerFr  VARCHAR(8),
			    		l_UkerTo  VARCHAR(8),
			    		l_FCab    VARCHAR(4),	
			    		l_TCab    VARCHAR(4),
...
--
RETURNS TABLE (OUTNoUrut	INT,
		OUTTahun	INT,
		OUTBulan	INT,
		OUTArea		VARCHAR(4),
		OUTSkArea	VARCHAR(20),
		OUTCabang	VARCHAR(4),
		OUTSkCabang	VARCHAR(20),
		OUTUnitUsaha	VARCHAR(4),
		OUTSkUUsa	VARCHAR(20),
		OUTKetUUsa	VARCHAR(20),
		OUTKdUker	VARCHAR(8),
		OUTSkUker	VARCHAR(20), 
		OUTGlng		VARCHAR(4), 
		OUTSkGlng	VARCHAR(20), 
		OUTKJab		VARCHAR(4), 
		OUTSkKjab	VARCHAR(20), 
		OUTJaba		VARCHAR(4), 
		OUTSkJaba	VARCHAR(20), 
		OUTKlas		VARCHAR(4), 
		OUTSkKlas	VARCHAR(20), 
		OUTNip		VARCHAR(10),
		OUTHari		DECIMAL(4,1), 
		...
		OUTKetTaxCal	VARCHAR(45),
		OUTNilaiTaxCal	DECIMAL(15,2),
		OUTJnsPjk	VARCHAR(1),
		OUTJmlPiutang	DECIMAL(15,2),
		OUTFlagThr	INT,
		OUTHarian	INT,
		OUTJmlLembur  	DECIMAL(5,1),
		OUTFLGOPBL 	VARCHAR(12))
AS $$
--
DECLARE l_Para			VARCHAR(4000);	l_LineTXT		VARCHAR(4000);
	l_S02TglPayr		DATE;		l_S01kdArea		VARCHAR(4);
	l_S01kdCaba		VARCHAR(4);	l_S01kdUUsa		VARCHAR(4);
	l_S01kdUker		VARCHAR(8);	l_S01kdGlng		VARCHAR(4);
	l_S01kdKJab		VARCHAR(4);	l_S01kdJaba		VARCHAR(4);
	l_S01kdKlas		VARCHAR(4);	l_S01NIP		VARCHAR(10);
	l_S01Nama		VARCHAR(25);	l_S01JnsKlmn		VARCHAR(1);
	l_S01StsPjk		VARCHAR(2);	l_S02FlgDpPt		VARCHAR(1);
	l_M19NomFormat		DECIMAL(3,0);	l_M01Singkatan		VARCHAR(20);
	l_M08SkCaba		VARCHAR(20);	l_M02Singkatan		VARCHAR(20);
	l_M17Singkatan		VARCHAR(20);	l_M12Singkatan		VARCHAR(20);
	l_M06Singkatan		VARCHAR(20);	l_M04Singkatan		VARCHAR(20);
	l_M10Singkatan		VARCHAR(20);	l_M19Keterangan		VARCHAR(20);
	l_M02Keterangan		VARCHAR(20);	l_S02Nilai		DECIMAL(15,2);
	l_S01BankRef		VARCHAR(4);	l_S01AccHolder		VARCHAR(25);
	l_S01RkBnk		VARCHAR(20);	l_M14Keterangan		VARCHAR(40);
	l_S01JnsPajak		VARCHAR(1);	l_S01FlagTHR		VARCHAR(1);
	l_GetPiut		Decimal(15,2);	l_TEMPTahun		INT;
	l_TEMPBulan		INT;		l_TEMPArea		VARCHAR(4);
	l_TEMPSkArea		VARCHAR(20);	l_TEMPCabang		VARCHAR(4);
	l_TEMPSkCabang		VARCHAR(20);	l_TEMPUnitUsaha		VARCHAR(4);
	l_TEMPSkUUsa		VARCHAR(20);	l_TEMPKetUUsa		VARCHAR(20);
	l_TEMPKdUker		VARCHAR(8);	l_TEMPSkUker		VARCHAR(20); 
	l_TEMPGlng		VARCHAR(4);	l_TEMPSkGlng		VARCHAR(20); 
	l_TEMPKJab		VARCHAR(4);	l_TEMPSkKjab		VARCHAR(20); 
	l_TEMPJaba		VARCHAR(4);	l_TEMPSkJaba		VARCHAR(20); 
	l_TEMPKlas		VARCHAR(4);	l_TEMPSkKlas		VARCHAR(20); 
	l_TEMPNip		VARCHAR(10);    l_TEMPFlagDppt		VARCHAR(1);
        l_TEMPNomFormat		DECIMAL(3,0);	l_TEMPNama		VARCHAR(25); 
	l_TEMPJnsKlm		VARCHAR(1);     l_TEMPStsMed		VARCHAR(2);
	l_TEMPKdBank		VARCHAR(4);	l_TEMPAccHolder		VARCHAR(25);
	l_TEMPAccNumber		VARCHAR(20);	l_TEMPKetBank		VARCHAR(40);
	l_TEMPKeterangan 	VARCHAR(20);	l_TEMPNilai		DECIMAL(15,2);
	l_TEMPJnsPjk		VARCHAR(1);	l_TEMPJmlPiutang 	DECIMAL(15,2);
	l_TEMPFlagThr		INT;    	l_LOOP_TEMP		REFCURSOR;
	l_Nomor			INT;    	l_TpFormat2		VARCHAR(1);
	l_KetTaxCal		VARCHAR(45);	l_NilTaxCal		DECIMAL(15,2);
	l_S01GrossIncomeNBYTD	DECIMAL(15,2);	l_S01OccSupport1	DECIMAL(15,2);
	l_S01Col12YTD		DECIMAL(15,2);	l_S01EGIYNB		DECIMAL(15,2);
	l_S01PTKP		DECIMAL(15,2);	l_S01EYTI		DECIMAL(15,2);
	l_S01EYIT		DECIMAL(15,2);	l_S01YTDIT		DECIMAL(15,2);
	l_S01IncTaxAPaidNB	DECIMAL(15,2);	l_S01IncTaxAPaidB	DECIMAL(15,2);
	l_S01GrossIncomeBYTD	DECIMAL(15,2);	l_S01OccSupport2	DECIMAL(15,2);
	l_S01TaxTotal		DECIMAL(15,2);	l_NIP			VARCHAR(10);
	l_TotPend		DECIMAL(15,2);	l_TotPot		DECIMAL(15,2);
	l_S02KdDpPt		VARCHAR(4);	l_S01Hari		DECIMAL(4,1);
	l_M10Harian		INT;            l_Nilai                 DECIMAL(15,2);
        l_S01TaxPesangonRp      DECIMAL(15,2);  l_FlgDpPt               VARCHAR(1);
	l_S03TaxUMP		DECIMAL(15,2);  l_S03TaxAPaidUMP  	DECIMAL(15,2);
	l_JmlLembur 		DECIMAL(5,1);	l_LOOP_W11		REFCURSOR;
...
CREATE TEMP TABLE l_TEMP1 (NoUrut	INT,
			...
			FlagThr		INT,
			Harian		INT,
			JmlLembur  	DECIMAL(5,1)) ON COMMIT DROP ;          
--*
-- Mulai Cursor data
OPEN l_LOOP_W11 FOR 
...
ON S02.TglPayr=S01.TglPayr AND
   S02.NIP    =S01.NIP
--
LEFT JOIN S03LTAX S03
ON S03.TglPayr=S01.TglPayr AND
   S03.NIP    =S01.NIP
--
...
INTO    l_S02TglPayr,l_S01kdArea,l_S01kdCaba,l_S01kdUUsa,l_S01kdUker,l_S01kdGlng,l_S01kdKJab,
	...
	EXIT ;
     ...
     ELSE
	--
          INSERT INTO l_TEMP (Tahun                          ,Bulan                           ,Area       ,SkArea        ,Cabang     ,SkCabang   ,UnitUsaha  ,SkUUsa        ,KdUker     ,SkUker        ,Glng       ,SkGlng        ,KJab       ,SkKjab        ,Jaba       ,SkJaba        ,Klas       ,SkKlas        ,Nip     ,FlagDppt    ,NomFormat     ,Nama     ,JnsKlm      ,StsMed     ,KdBank      ,AccHolder     ,AccNumber ,KetBank        ,Keterangan     ,Nilai  ,JnsPjk       ,JmlPiutang, FlagTHR                                    ,ketUUsa)
		      VALUES (EXTRACT(YEAR FROM l_S02TglPayr),EXTRACT(MONTH FROM l_S02TglPayr),l_S01kdArea,l_M01Singkatan,l_S01kdCaba,l_M08SkCaba,l_S01kdUUsa,l_M02Singkatan,l_S01kdUker,l_M17Singkatan,l_S01kdGlng,l_M12Singkatan,l_S01kdKJab,l_M06Singkatan,l_S01kdJaba,l_M04Singkatan,l_S01kdKlas,l_M10Singkatan,l_S01NIP,l_S02FlgDpPt,l_M19NomFormat,l_S01Nama,l_S01JnsKlmn,l_S01StsPjk,l_S01BankRef,l_S01AccHolder,l_S01RkBnk,l_M14Keterangan,l_M19Keterangan,l_Nilai,l_S01JnsPajak, l_GetPiut,CASE l_S01FlagThr WHEN '*' THEN 1 ELSE 0 END,l_M02Keterangan);
	--	
     END IF; 	
     --*
     -- Isi Data TEMP1 
	 ...
	 WHERE Tahun=EXTRACT(YEAR FROM l_S02TglPayr) AND 
          ...
          LOOP 
		     ...
		     l_NiltaxCal := l_S03TaxUMP-l_S03TaxAPaidUMP;
                ...
                INSERT INTO l_TEMP1(NoUrut,Tahun                          ,Bulan                           ,Area       ,SkArea        ,Cabang     ,SkCabang   ,UnitUsaha  ,SkUUsa        ,KdUker     ,SkUker        ,Glng       ,SkGlng        ,KJab       ,SkKjab        ,Jaba       ,SkJaba        ,Klas       ,SkKlas        ,Nip     ,FlagDppt    ,NomFormat     ,Nama     ,JnsKlm      ,StsMed     ,KdBank      ,AccHolder     ,AccNumber ,KetBank        ,Keterangan  ,Nilai,JnsPjk       ,JmlPiutang , FlagTHR                                    ,ketUUsa         ,TpFormat2  ,KetTaxCal   ,NilaiTaxCal ,Tpformat1,Hari     ,Harian     ,JmlLembur)
	       	            VALUES(l_Nomor,EXTRACT(YEAR FROM l_S02TglPayr),EXTRACT(MONTH FROM l_S02TglPayr),l_S01kdArea,l_M01Singkatan,l_S01kdCaba,l_M08SkCaba,l_S01kdUUsa,l_M02Singkatan,l_S01kdUker,l_M17Singkatan,l_S01kdGlng,l_M12Singkatan,l_S01kdKJab,l_M06Singkatan,l_S01kdJaba,l_M04Singkatan,l_S01kdKlas,l_M10Singkatan,l_S01NIP,l_S02FlgDpPt,l_M19NomFormat,l_S01Nama,l_S01JnsKlmn,l_S01StsPjk,l_S01BankRef,l_S01AccHolder,l_S01RkBnk,l_M14Keterangan,'         ' ,0    ,l_S01JnsPajak, l_GetPiut ,CASE l_S01FlagThr WHEN '*' THEN 1 ELSE 0 END,l_M02Keterangan ,l_TpFormat2,l_KetTaxCal ,l_NiltaxCal ,'T'      ,l_S01Hari,l_M10Harian,l_JmlLembur);
                --
                l_Nomor := l_Nomor+1; 
                --                
          END LOOP; 
...
SELECT Tahun,Bulan,Area,SkArea,Cabang,SkCabang,UnitUsaha,SkUUsa,KetUUsa,KdUker,SkUker,Glng,SkGlng,KJab,SkKjab,Jaba,SkJaba,Klas,SkKlas,Nip,FlagDppt,NomFormat,Nama,JnsKlm,StsMed,KdBank,AccHolder,AccNumber,KetBank,Keterangan,Nilai,JnsPjk,JmlPiutang,FlagThr
FROM l_TEMP
ORDER BY Tahun,Bulan,Area,SkArea,Cabang,SkCabang,UnitUsaha,SkUUsa,KetUUsa,KdUker,SkUker,Glng,SkGlng,KJab,SkKjab,Jaba,SkJaba,Klas,SkKlas,Nip,FlagDppt,NomFormat;
<<l_LOOP_TEMP>> 

LOOP 
FETCH l_LOOP_TEMP 
INTO l_TEMPTahun,l_TEMPBulan,l_TEMPArea,l_TEMPSkArea,l_TEMPCabang,l_TEMPSkCabang,l_TEMPUnitUsaha,l_TEMPSkUUsa,l_TEMPKetUUsa,l_TEMPKdUker,l_TEMPSkUker,l_TEMPGlng,l_TEMPSkGlng,l_TEMPKJab,l_TEMPSkKjab,l_TEMPJaba,l_TEMPSkJaba,l_TEMPKlas,l_TEMPSkKlas,l_TEMPNip,l_TEMPFlagDppt,l_TEMPNomFormat,l_TEMPNama,l_TEMPJnsKlm,l_TEMPStsMed,l_TEMPKdBank,l_TEMPAccHolder,l_TEMPAccNumber,l_TEMPKetBank,l_TEMPKeterangan,l_TEMPNilai,l_TEMPJnsPjk,l_TEMPJmlPiutang,l_TEMPFlagThr;
...
--
     ...
     IF l_TEMPFlagDppt='D' AND l_TEMPFlagDppt<>l_FlgDpPt THEN 
	  ...
	  SET TpFormat1 ='T',
	      ...
	      Nilai     =0	
	  WHERE NIP=l_TEMPNip  AND NoUrut=l_Nomor;    
          --
          l_Nomor := l_Nomor+1;
          UPDATE l_TEMP1
	  SET TpFormat1 ='D',
...
/*
SELECT * FROM P_SLIP_A ('2013-01-28'::DATE,'','ZZ','','ZZ','','ZZ','PHW-001', 'PHW-001','Copyright, 1988 (c) Microsoft Corporation, All rights reserved');

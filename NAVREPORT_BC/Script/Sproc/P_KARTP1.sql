...
Sub sprocs	: -
*****************************************/
DROP FUNCTION P_KARTP1(l_Tahun	INT,
			l_NIPFr		VARCHAR(10),
			l_NIPTo		VARCHAR(10),
			l_MyPass 	VARCHAR(128),
			l_Usr_Id	INT);     
--
CREATE FUNCTION P_KARTP1(l_Tahun	INT,
			l_NIPFr		VARCHAR(10),
			l_NIPTo		VARCHAR(10),
			l_MyPass 	VARCHAR(128),
			l_Usr_Id	INT)     
...
CREATE TEMP TABLE l_TEMP (Tahun		VARCHAR(4),
			Bulan		VARCHAR(2),
			Area		VARCHAR(4),
			SkArea		VARCHAR(10),
			Cabang		VARCHAR(4),
...
SELECT  PTKPPay,   PTKPDep,   TunjAnk
INTO    l_PTKPPay, l_PTKPDep, l_TunjAnk
FROM FZ1FLDA; 
--
-- Mulai Cursor data
OPEN l_LOOP_W07 FOR
...
ON S02.TglPayr=S01.TglPayr AND
   S02.NIP    =S01.NIP
--
INNER JOIN V_HGAJ VG
ON FN_CARITGLPAYR(S02.NIP, S02.TglPayr) = VG.TglPayr AND
   S02.NIP	= VG.NIP
--
...
--      S01.KdGlng BETWEEN l_FGol AND l_TGol AND
      EXTRACT(YEAR FROM S01.TglPayr)=l_Tahun ;
...
--      
/*
      IF SUBSTRING(l_S01StsPjk,1,1)='K'
         BEGIN
	   SET l_PTKP=l_PTKPPay+(l_PTKPDep*(l_TunjAnk+1))
	 END
      ELSE
         BEGIN
	   SET l_PTKP=l_PTKPPay+(l_PTKPDep*l_TunjAnk)
	 END
*/
-- Ambil Nilai PTKP
      ...
      --
      IF (SELECT COUNT(*) FROM l_TEMP WHERE Tahun=EXTRACT(YEAR FROM l_S02TglPayr)::VARCHAR(4) AND Area=l_S01kdArea 
	  ...
	  --	
     ELSE
           INSERT INTO l_TEMP(Tahun             ,Bulan              ,Area       ,SkArea        ,Cabang     ,SkCabang   ,UnitUsaha  ,SkUUsa        ,KetUUsa        ,KdUker     ,SkUker        ,Glng       ,SkGlng        ,KJab       ,SkKjab        ,Jaba       ,SkJaba        ,Klas       ,SkKlas        ,Nip     ,Nama     ,JnsKlm      ,StsMed     ,FlagDppt    ,NomFormat     ,Keterangan     ,Nilai,PTKP  ,Bln01,Bln02,Bln03,Bln04,Bln05,Bln06,Bln07,Bln08,Bln09,Bln10,Bln11,Bln12)
		                VALUES(EXTRACT(YEAR FROM l_S02TglPayr),l_Bulan,l_S01kdArea,l_M01Singkatan,l_S01kdCaba,l_M08SkCaba,l_S01kdUUsa,l_M02Singkatan,l_M02Keterangan,l_S01kdUker,l_M17Singkatan,l_S01kdGlng,l_M12Singkatan,l_S01kdKJab,l_M06Singkatan,l_S01kdJaba,l_M04Singkatan,l_S01kdKlas,l_M10Singkatan,l_S01NIP,l_S01Nama,l_S01JnsKlmn,l_S01StsPjk,l_S02FlgDpPt,l_M19NomFormat,l_M19Keterangan,0    ,l_PTKP,
...
$$ LANGUAGE plpgsql ;
/*
SELECT * FROM P_KARTP1(2013,' ','ZZZZ','Copyright, 1988 (c) Microsoft Corporation, All rights reserved',1) ;  
*/

/******************************************
Name sprocs	: P_Hit_JKM
Create by	: Wati
Date		: 19-06-2003
Description	: Proses Hitung JKM
Call From	: Main sprocs
*****************************************/
DROP FUNCTION P_Hit_JKM(l_TglProses  DATE,
				     l_NIP        VARCHAR(10),
				     l_MyPass    VARCHAR(128),
				     l_S01_ID    INT);      
--
CREATE FUNCTION P_Hit_JKM(l_TglProses  DATE,
				     l_NIP        VARCHAR(10),
				     l_MyPass    VARCHAR(128),
				     l_S01_ID    INT)      
RETURNS VOID 
AS $$
DECLARE l_PersenJKM    DECIMAL(6,3);
        l_NilaiJKM     DECIMAL(15,2);
        l_NilaiJKMVal  DECIMAL(15,2);
...
SELECT COALESCE(fn_Vround((TBL.FixIncome*l_PersenJKM)/100),0)
INTO   l_NilaiJKM
FROM
(
...
from  P_M03AllField ('D', 'JKM' ) fx
INTO    l_M03Singkatan, l_M03UsComp, l_M03Kolom, l_M03NoAcc, l_M03Status, 
	l_M03Persen, l_M03NiLai, l_M03NoLyr, l_M03KdCurr ;
-- Get Kurs
l_NilaiJKMVal := COALESCE(fn_Vround(l_NilaiJKM/fn_GetKurs(l_M03KdCurr,l_TglProses)),0);

-- Insert dan Update untuk Pendapatan JKM
IF (SELECT COUNT(NIP) FROM S02DGAJ WHERE NIP=l_NIP AND TglPayr=l_TglProses AND 
          FlgDpPt='D' AND KdDpPt='JKM' AND COALESCE(FlgAngs,' ')=' ') > 0 THEN 
   BEGIN
     UPDATE S02DGAJ
     SET EncNilaiVal = fn_kcabang(l_NIP,(fn_kpusat(l_NIP,EncNilaiVal,l_MyPass) ::DECIMAL(15,2)+l_NilaiJKMVal)  ::VARCHAR(17),l_MyPass),
         EncNilai    = fn_kcabang(l_NIP,(fn_kpusat(l_NIP, EncNilai,l_MyPass) :: Decimal(15,2)+l_NilaiJKM) :: VARCHAR(17),l_MyPass)
     WHERE NIP=l_NIP AND TglPayr=l_TglProses AND FlgDpPt='D' AND KdDpPt='JKM' AND COALESCE(FlgAngs,' ')=' ';
   END;
ELSE
   BEGIN
     INSERT INTO S02DGAJ(TglPayr    ,NIP  ,FlgDpPt,KdDpPt,FlgAngs,Singkatan ,    EncNilai                                              ,Nilai,KdCurr     ,EncNilaiVal                                              ,NilaiVal,Kolom     ,NoAcc ,    UsComp ,    NoLyr,     FlgNonGL, S01_ID)
                  VALUES(l_TglProses,l_NIP,'D'    ,'JKM', ' '    ,l_M03Singkatan,fn_kcabang(l_NIP,(l_NilaiJKM :: VARCHAR(17)),l_MyPass),0    ,l_M03KdCurr,fn_kCabang(l_NIP,(l_NilaiJKMVal :: VARCHAR(17)),l_MyPass),0       ,l_M03Kolom,l_M03NoAcc,l_M03UsComp,l_M03NoLyr,0,        l_S01_ID);	 
   END;
END IF; 
...
SELECT fx.l_Singkatan, fx.l_UsComp, fx.l_Kolom, fx.l_NoAcc, fx.l_STS,
       fx.l_PRS, fx.l_NL, fx.l_FLBR, fx.l_kdCurrT
from  P_M03AllField ('P', 'JKM' ) fx
INTO    l_M03Singkatan, l_M03UsComp, l_M03Kolom, l_M03NoAcc, l_M03Status, 
	l_M03Persen, l_M03NiLai, l_M03NoLyr, l_M03KdCurr ;	

IF(SELECT COUNT(NIP) FROM S02DGAJ WHERE NIP=l_NIP AND TglPayr=l_TglProses AND 
          FlgDpPt='P' AND KdDpPt='JKM' AND COALESCE(FlgAngs,' ')=' ') > 0 THEN 
   BEGIN
     UPDATE S02DGAJ
     SET EncNilaiVal =fn_kcabang(l_NIP,(fn_kpusat(l_NIP,EncNilaiVal,l_MyPass) ::DECIMAL(15,2)+l_NilaiJKMVal) ::VARCHAR(17),l_MyPass),
         EncNilai    =fn_kcabang(l_NIP,(fn_kpusat(l_NIP, EncNilai,l_MyPass) :: Decimal(15,2)+l_NilaiJKM) :: VARCHAR(17),l_MyPass)
     WHERE NIP=l_NIP AND TglPayr=l_TglProses AND FlgDpPt='P' AND KdDpPt='JKM' AND COALESCE(FlgAngs,' ')=' ';
   END;
ELSE
   BEGIN
     INSERT INTO S02DGAJ(TglPayr    ,NIP ,FlgDpPt,KdDpPt,FlgAngs,Singkatan ,    EncNilai                                              ,Nilai,KdCurr     ,EncNilaiVal                                              ,NilaiVal,Kolom     ,NoAcc ,    UsComp ,    NoLyr,     FlgNonGL, S01_ID)
                  VALUES(l_TglProses,l_NIP,'P'    ,'JKM', ' '   ,l_M03Singkatan,fn_kcabang(l_NIP,(l_NilaiJKM :: VARCHAR(17)),l_MyPass),0    ,l_M03KdCurr,fn_kCabang(l_NIP,(l_NilaiJKMVal :: VARCHAR(17)),l_MyPass),0       ,l_M03Kolom,l_M03NoAcc,l_M03UsComp,l_M03NoLyr,0       , l_S01_ID);	 
   END;
END IF; 
END;
$$ LANGUAGE plpgsql ;

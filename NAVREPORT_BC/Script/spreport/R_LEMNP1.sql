 /****************************************
Name sprocs	: R_LEMNP1
Create by	: Deni
Date		: 06-09-2002
Description	: Laporan Lembur per NIP (dipanggil untuk Report r_lemnp1, r_lemnp2, r_lemnp3, r_lemnp4)
Call From	: lemnS1
*****************************************/
DROP FUNCTION  R_LEMNP1(l_TglFr	DATE,
             l_TglTo	DATE,	
             l_NIPFr	VARCHAR(10),
             l_NIPTo	VARCHAR(10),
			 l_cabFr    VARCHAR(4),
             ...
             l_UserId   INT);
--
CREATE FUNCTION  R_LEMNP1(l_TglFr	DATE,
             l_TglTo	DATE,	
             l_NIPFr	VARCHAR(10),
             l_NIPTo	VARCHAR(10),
			 l_cabFr    VARCHAR(4),
...
BEGIN 
    ...
    IF l_JnsLap='D' THEN        
        ...
        SELECT 	S04.NIP, S04.TglPayr, S04.KdArea, S04.KdCaba, S04.KdUUsa, S04.KdUker, 
                ...
                S04.Nilai, S04.Perhitungan1, S04.Perhitungan2, S04.Perhitungan3, l_hrkerja,
        ...
        WHERE (S04.TglPayr BETWEEN l_TglFr AND l_TglTo) AND
              (S04.NIP     BETWEEN l_NIPFr AND l_NIPTo) AND (S04.KdCaba BETWEEN l_cabFr AND l_cabTo);
    ELSE      
        RETURN QUERY
        SELECT 	S04.NIP, NULL :: DATE AS TglPayr, S04.KdArea, S04.KdCaba, S04.KdUUsa, S04.KdUker, 
                ...
                l_hrkerja, S04.Pendek, S04.DasarLembur, S04.LembI1, S04.LembI2
        FROM S04LEMB S04 
        INNER JOIN M01AREA M01 ON S04.KdArea=M01.Kode
        INNER JOIN M02UUSA M02 ON S04.KdUUsa=M02.Kode
        INNER JOIN M17UKER M17 ON S04.KdUker=M17.KdUker
...
--
/*
SELECT * FROM R_LEMNP1 ('2012-01-01', '2012-01-31', '', '200601001',' ', 'ZZZ', 'R', 22, 1)
SELECT * FROM R_LEMNP1 ('2012-01-01', '2012-01-31', '', '200601001',' ', 'ZZZ', 'D', 22, 1)

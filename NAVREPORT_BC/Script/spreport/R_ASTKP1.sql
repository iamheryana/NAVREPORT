/****************************************
Name sprocs	: R_ASTKP1
Create by	: Deni
Date		: 28-01-2014
Description	: Proses Untuk Report ASTEK
Call From	: Main Proc
Sub sprocs	: -
*****************************************/
DROP FUNCTION R_ASTKP1 (l_Periode     DATE,
                        l_Pilih         VARCHAR(1),
						l_Mypass		VARCHAR(128),
						l_UserId		INT);
--
CREATE FUNCTION R_ASTKP1 (l_Periode     DATE,
                        l_Pilih         VARCHAR(1),
						l_Mypass		VARCHAR(128),
						l_UserId		INT)
--
...
BEGIN 
    ...
    FROM 
    (--Q
    SELECT  S02.NIP, M15.KPA AS NoAstek, M15.Nama, M15.Status, M15.KdJaba, M04.Keterangan AS NmJaba, M15.TglMasuk,
            S02.FlgDpPt, S02.KdDpPt, M15.JKK, M15.JKM, M15.JHT, M15.JPK, M15.KdUUsa,
    ...
    WHERE S02.FlgDpPt='D' AND M15.KPA <> ' ' AND M15.TGLKPA IS NOT NULL AND 
          ...
          ((TO_CHAR(S02.TglPayr,'YYYYMM') = TO_CHAR(l_Periode,'YYYYMM') AND  l_Pilih='P') OR
           (TO_CHAR(S02.TglPayr,'YYYYMM') BETWEEN  TO_CHAR(l_Periode,'YYYY01')  AND TO_CHAR(l_Periode,'YYYYMM') AND  l_Pilih<>'P')) 
    ) Q 
    ) Q1
...
select  TO_CHAR(current_date,'YYYY13DD') ;

SELECT EXTRACT(MONTH FROM CURRENT_DATE);
Result: 2

select * from R_ASTKP1('2013-11-08','X', 'Copyright, 1988 (c) Microsoft Corporation, All rights reserved', 1)
*/

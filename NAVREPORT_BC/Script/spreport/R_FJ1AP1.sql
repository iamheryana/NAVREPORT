/****************************************
Name sprocs	: R_FJ1AP1
Create by	: Deni
Date		: 28-02-2003
Description	: Query Pendaftaran Tenaga Kerja
Call From	: Main Proc
*****************************************/
DROP FUNCTION R_FJ1AP1(l_Periode      DATE,
                        l_FCab          VARCHAR(4),
                        l_TCab          VARCHAR(4),
    					l_mlCetak       VARCHAR(1),
						...
						l_UserId		INT);
--
CREATE FUNCTION R_FJ1AP1 (  l_Periode       DATE,
                            l_FCab          VARCHAR(4),
                            l_TCab          VARCHAR(4),
                            l_mlCetak       VARCHAR(1),
                            l_mcJudulCab	VARCHAR(4),
...
BEGIN
    ...
    FROM FZ2FLDA; 
    -- 
    IF l_mcPindah = '1' AND l_mlCetak = 'T' THEN
    ...
    SELECT  Q1.NIP, Q1.KPA, Q1.Nama, Q1.TglLahir, Q1.JnsKlmn, Q1.Gaji,
            (CASE WHEN l_mlCetak='Y' THEN Q1.KdCaba  ELSE l_KdCabaJudul END) AS KdCaba, 
            (CASE WHEN l_mlCetak='Y' THEN M08.NmCaba ELSE l_NmCabaJudul END) AS NmCaba, 
            (CASE WHEN l_mlCetak='Y' THEN M08.NamaJamsostek ELSE l_TTanganJudul END) AS TandaTangan, 
            (CASE WHEN l_mlCetak='Y' THEN M08.JabatanJamsostek ELSE l_JabatanJudul END) AS Jabatan, 
            (CASE WHEN l_mlCetak='Y' THEN M08.NPP ELSE l_NPPJudul END) AS NPP
    FROM 
    (--Q1 
    -- PEG BARU 
    SELECT M15.NIP, M15.KPA, M15.Nama, M15.TglLahir, M15.JnsKlmn, M15.KdCaba, 
    ...
    WHERE  TO_CHAR(M15.TglKPA,'YYYYMM') = TO_CHAR(l_Periode,'YYYYMM') AND
           (M15.KdCaba BETWEEN l_FCab AND l_TCab) AND 
           M15.KPA <> ' ' AND M15.TGLKPA IS NOT NULL AND M03.Status = '1' 
    GROUP BY M15.NIP, M15.KPA, M15.Nama, M15.TglLahir, M15.JnsKlmn, M15.KdCaba
    --------- 
    UNION ALL 
    --------- 
    -- PINDAH CABANG 
    ...
    WHERE TO_CHAR(M15.TglKPA,'YYYYMM') <> TO_CHAR(l_Periode,'YYYYMM') AND
          ...
          (M15.KdCaba BETWEEN l_FCab AND l_TCab) AND 
          ((l_mlCetak = 'Y' AND S01.KdCaba <> X01.KdCaba) OR 
           (l_mlCetak = 'T' AND S01.KdCaba <> X01.KdCaba) AND (S01.KdCaba BETWEEN l_FCab AND l_TCab)) 
...
select * from R_FJ1AP1('2013-11-08',' ','ZZZ','Y','001','Copyright, 1988 (c) Microsoft Corporation, All rights reserved', 1)
*/
